...
function TargetProfileModal({ target, profile, tasks, close, done, fail }: { target: ModelTarget; profile: TargetRoutingProfile | null; tasks: RoutingTaskDefinition[]; close: () => void; done: () => Promise<void>; fail: (error: unknown) => void }) {
  const local = target.kind === "gguf" || target.kind === "mlx";
  ...
  return <Modal title={`Routing profile · ${target.name}`} close={close}><form className="form" onSubmit={submit}><div className="two-fields"><Field label="Context window"><input type="number" min="1" value={value.context_window} onChange={event => setValue({ ...value, context_window: Number(event.target.value) })} /></Field><Field label="Latency prior ms"><input type="number" min="1" value={value.latency_prior_ms} onChange={event => setValue({ ...value, latency_prior_ms: Number(event.target.value) })} /></Field></div><div className="two-fields"><Field label="Input USD / 1M"><DecimalInput optional min={0} value={value.input_price_per_million} onChange={next => setValue({ ...value, input_price_per_million: next })} /></Field><Field label="Output USD / 1M"><DecimalInput optional min={0} value={value.output_price_per_million} onChange={next => setValue({ ...value, output_price_per_million: next })} /></Field></div><Field label="Reliability prior (0–1)"><input type="number" min="0" max="1" step="0.01" value={value.reliability_prior} onChange={event => setValue({ ...value, reliability_prior: Number(event.target.value) })} /></Field><div><span className="field-label">Quality by task (0–100)</span><div className="quality-grid">{tasks.map(task => <label key={task.id}><span>{task.label}</span><input aria-label={`${task.label} quality`} type="number" min="0" max="100" value={value.task_quality[task.id] ?? ""} onChange={event => { const next = { ...value.task_quality }; if (event.target.value) next[task.id] = Number(event.target.value); else delete next[task.id]; setValue({ ...value, task_quality: next }); }} /></label>)}</div></div>{(target.kind === "cloud" || target.kind === "uplink") && (value.input_price_per_million == null || value.output_price_per_million == null) && <div className="security-note warning-note"><CircleAlert size={17} /><span>Unknown pricing makes this target a last-resort candidate and prevents a guaranteed request budget.</span></div>}<ModalActions close={close} busy={busy} label="Save profile" /></form></Modal>;
}
...
function hopOrigin(target: ModelTarget, providers: Provider[]) {
  if (target.kind === "uplink") return { origin: "Uplink", detail: target.provider_model };
  if (target.kind === "replica") return { origin: "Network replica", detail: target.provider_model };
...
function SettingsPage({ settings, resourcePolicy, dashboard, refresh, success, fail }: Common & { dashboard: DashboardData }) {
  ...
  const [uplinkUrl, setUplinkUrl] = useState(""); const [uplinkUser, setUplinkUser] = useState(""); const [uplinkPass, setUplinkPass] = useState(""); const [uplinkSession, setUplinkSession] = useState(""); const [uplinkFingerprint, setUplinkFingerprint] = useState(""); const [uplinkBusy, setUplinkBusy] = useState(false);
  const uplinkQuery = useQuery({ queryKey: queryKeys.uplink, queryFn: fetchers.uplinkStatus });
  const uplink = uplinkQuery.data ?? null;
  const networkQuery = useQuery({ queryKey: queryKeys.networkModels, queryFn: fetchers.networkModels });
  const sharedQuery = useQuery({ queryKey: queryKeys.sharedImages, queryFn: fetchers.sharedImages });
  const parentImagesQuery = useQuery({ queryKey: queryKeys.parentSharedImages, queryFn: fetchers.parentSharedImages, enabled: !!uplink });
  const networkModels = networkQuery.data ?? [];
  const sharedImages = sharedQuery.data ?? [];
  const parentImages = parentImagesQuery.data ?? [];
  const [catalogId, setCatalogId] = useState("");
  const [catalogName, setCatalogName] = useState("");
  const [catalogRef, setCatalogRef] = useState("");
  const [catalogKind, setCatalogKind] = useState("huggingface");
  const [catalogArtifact, setCatalogArtifact] = useState("gguf");
  const [catalogRevision, setCatalogRevision] = useState("main");
  ...
  return <><PageHead eyebrow="Application" title="Settings" description="Security, resources and background behavior." />
    ...
    <div className="settings-list"><Setting title="Local endpoint" description={bindMode === "loopback" ? "The gateway is bound to 127.0.0.1 over HTTP. Enable LAN share to reach other machines." : "Non-loopback binds require HTTPS. Restart the app or serve process after changing bind settings."}><code>{dashboard.base_url}</code></Setting>
      ...
      <Setting title="Uplink parent" description="Join exactly one parent as a directory user (password or a parent session token from OpenID). For a self-signed LAN parent, pin the SHA-256 TLS fingerprint shown in the parent’s Settings. Granted public model IDs are mounted locally; the parent never sees this node's API keys.">
        {uplink ? <div className="resource-controls" style={{ flexDirection: "column", alignItems: "flex-end" }}>
          <span>Joined as <strong>{uplink.username}</strong> on <code>{uplink.base_url}</code>{uplink.may_publish ? " · may publish" : ""}</span>
          <span>{uplink.models.map(model => model.id).join(", ") || "no granted models"}</span>
          {uplink.quota && <span>
            {uplink.quota.rpm != null ? `${uplink.quota.rpm_used} / ${uplink.quota.rpm} RPM` : "no RPM cap"}
            {uplink.quota.daily_token_budget != null ? ` · ${uplink.quota.daily_tokens_used} / ${uplink.quota.daily_token_budget} tokens today` : ""}
            {uplink.quota.daily_usd_budget != null ? ` · $${uplink.quota.daily_usd_used.toFixed(4)} / $${uplink.quota.daily_usd_budget} today` : ""}
          </span>}
          <button className="secondary danger-text" disabled={uplinkBusy} onClick={async () => { setUplinkBusy(true); try { await command("disconnect_uplink"); await refresh(); success("Disconnected from uplink parent"); } catch (error) { fail(error); } finally { setUplinkBusy(false); } }}>Disconnect</button>
        </div> : <div className="inline-form">
          <input aria-label="Uplink parent URL" value={uplinkUrl} onChange={event => setUplinkUrl(event.target.value)} placeholder="https://router.local:11435" />
          <input aria-label="Uplink username" value={uplinkUser} onChange={event => setUplinkUser(event.target.value)} placeholder="username" />
          <input aria-label="Uplink password" type="password" value={uplinkPass} onChange={event => setUplinkPass(event.target.value)} placeholder="password" />
          <input aria-label="Uplink session token" value={uplinkSession} onChange={event => setUplinkSession(event.target.value)} placeholder="session token (OpenID)" />
          <input aria-label="Uplink TLS fingerprint" value={uplinkFingerprint} onChange={event => setUplinkFingerprint(event.target.value)} placeholder="TLS fingerprint (LAN HTTPS)" />
          <button className="secondary" disabled={uplinkBusy || !uplinkUrl || !(uplinkSession.trim() || (uplinkUser && uplinkPass))} onClick={async () => { setUplinkBusy(true); try { await command("join_uplink", { input: { baseUrl: uplinkUrl, username: uplinkUser || null, password: uplinkPass || null, sessionToken: uplinkSession.trim() || null, tlsFingerprint: uplinkFingerprint.trim() || null } }); setUplinkPass(""); setUplinkSession(""); await refresh(); success("Joined uplink parent"); } catch (error) { fail(error); } finally { setUplinkBusy(false); } }}>Join</button>
        </div>}
      </Setting>
      <Setting title="Live network models" description="Children with may_publish advertise a running local model. The parent exposes one public ID and fails over between healthy replicas. In-flight streams stay on the replica that accepted them. The child must be reachable (LAN HTTPS); the parent uses a replica session token, never the child's local API keys.">
        {networkModels.length ? networkModels.map(model => <div className="model-row" key={model.id}><div className="grow"><strong>{model.id}</strong><span>{model.replicas.map(replica => `${replica.child_node_id.slice(0, 8)} · ${replica.local_model_id} · ${replica.healthy ? "healthy" : "unhealthy"}`).join(" · ") || "no live replicas"}</span></div></div>) : <span>No live replicas on this node.</span>}
      </Setting>
      <Setting title="Shared image catalog" description="Hub sources (Hugging Face, CivitAI, curated catalog) store metadata only. Unique local imports store bytes on this parent so other nodes can pull a copy. Disconnecting a child drops its node row, not the catalog entry.">
        <div className="resource-controls" style={{ flexDirection: "column", alignItems: "flex-end" }}>
          {sharedImages.map(image => <div className="model-row" key={image.id}><div className="grow"><strong>{image.name}</strong><span>{image.source_kind} · {image.source_ref}{image.revision ? ` @ ${image.revision}` : ""} · {image.size_bytes != null ? formatBytes(image.size_bytes) : "metadata only"} · {image.nodes.length ? image.nodes.map(node => node.node_id.slice(0, 8)).join(", ") : "no nodes"}</span></div></div>)}
          <div className="inline-form">
            <select aria-label="Catalog source kind" value={catalogKind} onChange={event => setCatalogKind(event.target.value)}><option value="huggingface">Hugging Face</option><option value="civitai">CivitAI</option><option value="catalog">Local catalog</option><option value="local_blob">Unique local import</option></select>
            <select aria-label="Catalog artifact kind" value={catalogArtifact} onChange={event => setCatalogArtifact(event.target.value)}><option value="gguf">GGUF</option><option value="mlx">MLX</option><option value="image">Image</option><option value="speech">Speech</option></select>
            <input aria-label="Catalog image ID" value={catalogId} onChange={event => setCatalogId(event.target.value)} placeholder="llama-70b" />
            <input aria-label="Catalog image name" value={catalogName} onChange={event => setCatalogName(event.target.value)} placeholder="Llama 70B" />
            <input aria-label="Catalog image source" value={catalogRef} onChange={event => setCatalogRef(event.target.value)} placeholder={catalogKind === "local_blob" ? "local-import" : "org/llama-70b"} />
            {catalogKind !== "local_blob" && <input aria-label="Catalog revision" value={catalogRevision} onChange={event => setCatalogRevision(event.target.value)} placeholder="main" />}
            {catalogKind === "local_blob" && <input aria-label="Catalog local path" value={catalogPath} onChange={event => setCatalogPath(event.target.value)} placeholder="/path/to/model.gguf" />}
            <button className="secondary" disabled={!catalogId.trim() || !catalogName.trim() || !catalogRef.trim() || (catalogKind === "local_blob" && !catalogPath.trim())} onClick={async () => { try { await command("register_shared_image", { input: { id: catalogId, name: catalogName, sourceKind: catalogKind, sourceRef: catalogRef, revision: catalogKind === "local_blob" ? null : (catalogRevision.trim() || null), filename: null, kind: catalogArtifact, capabilities: catalogArtifact === "image" ? ["image"] : catalogArtifact === "speech" ? ["speech"] : ["chat"], localPath: catalogKind === "local_blob" ? catalogPath : null } }); setCatalogId(""); setCatalogName(""); setCatalogRef(""); setCatalogPath(""); await refresh(); success("Catalog entry registered"); } catch (error) { fail(error); } }}>Register image</button>
          </div>
          {!!parentImages.length && <>
            <span>From parent — hub sources install into Local models, then advertise with Offer to parent. Unique imports pull stored bytes.</span>
            {parentImages.map(image => <div className="model-row" key={image.id}><div className="grow"><strong>{image.name}</strong><span>{image.source_kind} · {image.source_ref}{image.revision ? ` @ ${image.revision}` : ""}{image.size_bytes != null ? ` · ${formatBytes(image.size_bytes)}` : ""}</span></div>{image.source_kind === "local_blob" ? <button className="secondary" onClick={async () => { try { await command("pull_shared_image", { input: { id: image.id } }); await refresh(); success(`Pulled ${image.name}`); } catch (error) { fail(error); } }}>Pull bytes</button> : <button className="secondary" onClick={async () => { try { await command("install_catalog_model", { input: { repoId: image.source_ref, revision: image.revision, catalogId: image.source_kind === "catalog" ? image.source_ref : null, confirmOverBudget: false, name: image.name } }); await command("report_shared_image_installed", { input: { id: image.id } }); await refresh(); success(`Installing ${image.name}`); } catch (error) { fail(error); } }}>Install</button>}</div>)}
          </>}
        </div>
      </Setting>
      ...
      <Setting title="GPU / NPU" description="GGUF GPU layers offload to Metal on macOS, CUDA on NVIDIA, or Vulkan on AMD (Linux/Windows). Set layers to 0 for CPU-only. MLX uses Metal; Apple Neural Engine quotas are not exposed. Override the backend with LOCAL_AI_ROUTER_GGUF_BACKEND=cpu|cuda|vulkan."><ResourceNumber label="GGUF layers" value={policy.gguf_gpu_layers} min={-1} max={999} suffix="-1 = auto" onChange={value => updatePolicy({ gguf_gpu_layers: value })} onSave={() => void savePolicy(policy)} /></Setting>
