    ...
    {tab !== "library" && tab !== "import" && <>
      <section className="panel catalog-toolbar">
        ...
        {tab === "image" && <div className="segmented small catalog-sources" role="group" aria-label="Download source">
          {([["huggingface", "Hugging Face"], ["civitai", "CivitAI"]] as const).map(([id, label]) =>
...
function ModelResourceEditor({ model, local, global, close, done, success, fail }: { model: ModelTarget; local: ModelTarget[]; global: ResourcePolicy; close: () => void; done: () => Promise<void>; success: (text: string) => void; fail: (error: unknown) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={close}><div className="modal" onMouseDown={event => event.stopPropagation()}><div className="modal-head"><h2>{model.name} resources</h2><button className="icon-button" onClick={close}>×</button></div><div className="form resource-override-form">
    ...
    {model.kind === "gguf" && <label className="field"><span>GPU layers (-1 = auto CUDA/Vulkan/CPU)</span><input type="number" min="-1" max="999" value={number("gguf_gpu_layers", global.gguf_gpu_layers)} onChange={event => setValues({ ...values, gguf_gpu_layers: Number(event.target.value) })} /></label>}
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (tab === "import") {
        ...
        const repoId = source.includes("civitai") ? source : `civitai/models/${source.replace(/^models\//, "")}`;
